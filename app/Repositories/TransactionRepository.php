...
{
    $transaction = Transaction::where('transaction_code', $kode)->first();
    ...
    // Jangan update transaction_type_id!
    $transaction->update([
        // 'transaction_type_id' => $request->transaction_type_id, // HAPUS/BLOCK BARIS INI
        'description' => $request->description ?? $transaction->description,
        'user_id' => $user->id,
        'transaction_date' => now(),
    ]);

    // Ambil semua detail lama sebelum dihapus
    $oldDetails = $transaction->transactionDetails()->get()->keyBy('barang_id');

    // Hapus semua item sebelumnya
    $transaction->transactionDetails()->delete();

    foreach ($request->items as $item) {
        $barang = \App\Models\Barang::where('barang_kode', $item['barang_kode'])->first();

        if (!$barang) {
            throw new \Exception("Barang dengan kode {$item['barang_kode']} tidak ditemukan.");
        }

        // Ambil gudang_id dari detail lama, fallback ke null jika tidak ada
        $gudangId = isset($oldDetails[$barang->id]) ? $oldDetails[$barang->id]->gudang_id : null;

        // Ambil data BarangGudang untuk validasi stok
        $barangGudang = null;
        if ($gudangId) {
            $barangGudang = \App\Models\BarangGudang::where('barang_id', $barang->id)
                ->where('gudang_id', $gudangId)
                ->first();
        }

        // Validasi stok & jenis transaksi seperti di create
        // Gunakan transaction_type_id dari transaksi lama, BUKAN dari request!
        $this->validateItemTransaction(
            $barang,
            $barangGudang,
            array_merge($item, ['gudang_id' => $gudangId]),
            $transaction->transaction_type_id // <-- pakai yang lama
        );
